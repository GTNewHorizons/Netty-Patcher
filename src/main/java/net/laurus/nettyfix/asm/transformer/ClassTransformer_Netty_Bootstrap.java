...
public class ClassTransformer_Netty_Bootstrap {
    ...
    protected final void init(String transformedName) {
        ...
        if (aFoundMethod) {
            ...
            injectMethod(writer);
            log(
                    "Found method \"checkAddress\", your version of Netty *IS* susceptible to MC-108343, this has been patched.");
        } else {
            // Logger.ASM("Unable to find method \"checkAddress\", your version of Netty is not susceptible to
            // MC-108343.");
        }
        if (aFoundMethod) {
    ...
    private boolean inject_checkAddress() {
        ...
        mv.visitMethodInsn(
                INVOKESTATIC,
                "net/laurus/nettyfix/utils/NettyUtils",
                "checkAddress",
                "(Lio/netty/bootstrap/Bootstrap;Ljava/net/SocketAddress;)Lio/netty/channel/ChannelFuture;",
                false);
