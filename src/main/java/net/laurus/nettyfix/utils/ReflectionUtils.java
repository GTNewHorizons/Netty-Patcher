...
public class ReflectionUtils {
    ...
    public static <T> T invokeNonBool(Object objectInstance, Method method, Object[] values) {
        ...
        Class<?> mLocalClass = (objectInstance instanceof Class ? (Class<?>) objectInstance
                : objectInstance.getClass());
        Logger.REFLECTION(
                "Trying to invoke " + method.getName() + " on an instance of " + mLocalClass.getCanonicalName() + ".");
        try {
            Logger.REFLECTION(method + " was not null.");
            return (T) method.invoke(objectInstance, values);
        } catch (SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
            Logger.REFLECTION(
                    "Failed to Dynamically invoke " + method.getName()
