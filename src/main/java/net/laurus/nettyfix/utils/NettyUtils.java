...
public class NettyUtils {
    ...
    public static ChannelFuture checkAddress(Bootstrap boostrap, SocketAddress remoteAddress) {
        if (remoteAddress instanceof InetSocketAddress) {
            ...
            if (address != null && (BlockedServers.isBlockedServer(address.getHostAddress())
                    || BlockedServers.isBlockedServer(address.getHostName()))) {
                Channel channel = ReflectionUtils.invokeNonBool(
                        boostrap,
                        ReflectionUtils.getMethod(boostrap.getClass(), "channelFactory", new Class[] {}),
                        new Object[] {});
                channel.unsafe().closeForcibly();
                SocketException cause = new SocketException("Network is unreachable");
                cause.setStackTrace(EmptyArrays.EMPTY_STACK_TRACE);
                return (ChannelFuture) (new DefaultChannelPromise(
                        channel,
                        (EventExecutor) GlobalEventExecutor.INSTANCE)).setFailure(cause);
            }
        }
        return null;
    }
    ...
    public boolean isBlockedServer(String server) {
        ...
        while (server.charAt(server.length() - 1) == '.') server = server.substring(0, server.length() - 1);
        if (isBlockedServerHostName(server)) return true;
        List<String> strings = Lists.newArrayList(DOT_SPLITTER.split(server));
        boolean isIp = (strings.size() == 4);
        if (isIp) for (String string : strings) {
            try {
        ...
        if (!isIp && isBlockedServerHostName("*." + server)) return true;
        while (strings.size() > 1) {
            strings.remove(isIp ? (strings.size() - 1) : 0);
            String starredPart = isIp ? (DOT_JOINER.join(strings) + ".*") : ("*." + DOT_JOINER.join(strings));
            if (isBlockedServerHostName(starredPart)) return true;
        }
        return false;
    }

    private boolean isBlockedServerHostName(String server) {
        return BLOCKED_SERVERS.contains(
                Hashing.sha1().hashBytes(server.toLowerCase().getBytes(Charset.forName("ISO-8859-1"))).toString());
    }
    ...
    public static class BlockedServers {
        ...
        static {
            try {
                BLOCKED_SERVERS.addAll(
                        IOUtils.readLines(
                                (new URL("https://sessionserver.mojang.com/blockedservers")).openConnection()
                                        .getInputStream(),
                                HASH_CHARSET));
        ...
        public static boolean isBlockedServer(String server) {
            if (server == null || server.isEmpty()) return false;
            if (server.startsWith(SRV_PREFIX)) server = server.substring(SRV_PREFIX.length());
            while (server.charAt(server.length() - 1) == '.') server = server.substring(0, server.length() - 1);
            if (isBlockedServerHostName(server)) return true;
        ...
        private static boolean isBlockedServerHostName(String server) {
            return BLOCKED_SERVERS
                    .contains(Hashing.sha1().hashBytes(server.toLowerCase().getBytes(HASH_CHARSET)).toString());
        }
    }
